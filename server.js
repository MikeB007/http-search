    ...
    if (fs.existsSync(keyPath) && fs.existsSync(certPath)) {
      ...
      if (fs.existsSync(autoGeneratedPFX)) {
        ...
        try {
          ...
          // Test if the certificate can be loaded by trying to create a secure context
          require('tls').createSecureContext(httpsOptions);
          hasSSL = true;
        } catch (certError) {
          console.log('⚠️  PKCS12 certificate compatibility issue detected:', certError.message);
          console.log('🔄 Trying PEM certificate fallback...');
          
          // Try to find PEM alternatives
          const pemKeyPath = path.join(__dirname, 'certs', 'private-key.pem');
          const pemCertPath = path.join(__dirname, 'certs', 'certificate.pem');
          
          if (fs.existsSync(pemKeyPath) && fs.existsSync(pemCertPath)) {
            console.log('✓ Using PEM certificate files');
            httpsOptions = {
            ...
            console.log('⚠️  No compatible certificates found. Will run HTTP-only mode.');
            hasSSL = false;
          }
        }
